            ...
            if step_idx < len(path):
                ...
                if (r, c) in next_positions:
                    ...
                    if command_count[bot_id] % 2 == 0:
                        log_text.insert(tk.END, f"{bot_id} is waiting to avoid collision.\n")
                        bot_paths[bot_id].insert(step_idx, path[step_idx - 1])  # Bot waits
                    else:
                        # Reverse and recalculate the path using A*
                        log_text.insert(tk.END, f"{bot_id} is reversing and recalculating path.\n")
                        ...
                        if new_path:
                            ...
                            log_text.insert(tk.END, f"{bot_id} found new path.\n")
                else:
                    ...
                    time_taken[bot_id] += 1
                    log_text.insert(tk.END, f"{bot_id} moved to {r}, {c}\n")
...
def create_grid():
    ...
    log_frame.grid(row=0, column=1, padx=10, pady=10)
